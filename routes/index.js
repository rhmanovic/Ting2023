...
router.get("/category/:category", async function (req, res, next) {
  ...
  try {
    ...
    const subCategoryData = await Category.find({
      parent: categoryData.thisCategory,
    });
  ...
  try {
    ...
    for (let i = 0; i < categoryData.length; i++) {
      if (categoryData[i].parent == "/") {
        ...
        var prouct = await Product.find({
          category: categoryData[i].URLname,
          showInWebsite: true,
        });
...
router.get("/upsellCategory/:productNo/:source", function (req, res, next) {
  ...
  Product.findOne({ productNo: productNo }).exec(function (error, productData) {
    if (error) {
      ...
      Product.find({ SuperProductID: productData._id }).exec(
        function (error, productSub) {
          if (error) {
            return next(error);
          } else {
...
router.get("/category/:category", function (req, res, next) {
  const { category } = req.params;
  //, status: "A"
  Product.find({ category: category, showInWebsite: true })
    .sort({ productNo: 1 })
    .exec(function (error, productData) {
      if (error) {
        return next(error);
      } else {
        Category.findOne({ URLname: category }, { name: 1 }).exec(
...
router.get("/sms", function (req, res, next) {
  client.messages
    .create(
      {
        body: "Thank your for order form ITC Store شكرا لطلبك",
        from: "+12149831341",
        to: "+96551755332",
      },
      function (err, data) {
...
router.get(
  "/editProductQuantite2/:productID/:newQuantity/:newPrice",
  function (req, res) {
...
router.get(
  "/editProductQuantite2/:productID/:newQuantity",
  function (req, res) {
    const { productID } = req.params;
    const { newQuantity } = req.params;
    var old = req.session.cartData0;
...
router.get(
  "/editProductQuantite/:productID/:newQuantity/:newPrice",
  function (req, res) {
    const { productID } = req.params;
    const { newQuantity } = req.params;
    const { newPrice } = req.params;
      ...
      if (req.session.cartData0) {
        old.forEach(function (product, index, array) {
          ...
          if (product.ID == productID) {
            ...
            data.totalPrice += (newPrice * 1000 * product.Quantity) / 1000;
          }
        });
      }
    } finally {
...
router.post("/AddOrder2", function (req, res, next) {
  ...
  Product.find(
    { _id: { $in: cartIDs } },
    { name: 1, price: 1, discountPrice: 1, cost: 1, warranty: 1, productNo: 1 },
  ).exec(function (error, productData) {
    cartData.forEach(function (product, index, array) {
...
router.post("/AddOrder", function (req, res1, next) {
  ...
  console.log(
    "orderData.shippingCost orderData.shippingCost orderData.shippingCost: " +
    ...
    function payforThis() {
      ...
      var req = http.request(options, function (res) {
        ...
        res.on("end", function () {
          ...
          console.log(
            "_______________________________________profile.transaction_______________________________________" +
              body.transaction,
          );
      ...
      res.on("end", async function () {
        ...
        Order.findOneAndUpdate({ _id: OrderId }, arr_update_dict).then(
          function () {
            ...
            return res1.render("redirectAfterPayment", {
              title: "Order",
              KentStatus: KentStatus,
              orderNo: orderData.orderNo,
            });
...
router.post("/cart", function (req, res, next) {
  ...
  var productName = referer.split("/")[4].split("?")[0];
  ...
  console.log(
    "Now the product is not in the cart we will make push + update session + redirect + cartCount",
  );
  old.push(newProduct);
  req.session.cartData0 = old;
  req.session.cartCount = req.session.cartData0.length;
  ...
  req.session.save(function (err) {
    ...
    if (buyknow) {
      ...
      return res.redirect(
        `https://${
          referer.split("/")[2]
        }/product/${productName}?ShowModal=yes&Q=${newProduct.Quantity}`,
      );
    }
  });
...
router.get("/Course/:id", function (req, res, next) {
  ...
  Course.findOne({ _id: id }).exec(function (error, courseData) {
    if (error) {
      ...
      var find = Chapter.find({ courseID: id })
        .sort({ order: 1 })
        .exec(function (error, ChaptersData) {
          try {
            var ChaptersDataEdited = [];

            if (req.session.userId) {
              var subscriptions = User.findById(req.session.userId).exec(
                function (error, user) {
                  if (error) {
                    return next(error);
                  } else {
...
router.get("/redirect2", function (req, res, next) {
  ...
  request(options, function (error, response, body) {
    ...
    if (bodyJSON.response) {
      ...
      Charge.findOne({ _id: bodyJSON.metadata.ChargeID }).exec(
        function (error, chargeData) {
          if (error) {
            return next(error);
          } else {
            if (status == "Captured") {
...
router.post("/pay", async function (req, res1, next) {
  ...
  Chapter.find({ _id: { $in: cartIDs } }, { name: 1, price: 1 }).exec(
      ...
      if (error) {
        ...
        for (var i = 0; i < productData.length; i++) {
          if (productData[i]._id == cartData[i].ID) {
            ...
            totalPrice +=
              (productData[i].price * 1000 * cartData[i].Quantity) / 1000;
          } else {
